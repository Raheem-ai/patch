...
echo git fetch --depth=10 --tags patch:Raheem-ai/patch.git $SHORT_SHA
git fetch --depth=10 --tags patch:Raheem-ai/patch.git $SHORT_SHA
...
changed_files=$(git diff --name-only $rc_commit $SHORT_SHA)
echo $changed_files
