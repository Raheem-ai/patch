...
# generate just config in a location that build scripts can consume them 
/app/backend/infra/bin/run config:generate -e $_ENVIRONMENT --toFile /workspace/build_config.env --noComments --config